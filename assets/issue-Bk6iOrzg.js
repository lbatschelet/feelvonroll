...
3. Error appears`,"email.label":"Email","email.hint":"(optional, for follow-up)","email.placeholder":"your@email.com","debug.summary":"Debug information (sent automatically)",submit:"Submit","error.required":"Please fill in the title and description.","error.generic":"Something went wrong. Please try again.","success.title":"Thank you!","success.message":"Your submission has been received.","success.another":"Submit another","footer.for":"for","footer.license":"Licensed under","footer.issues":"View all issues"}},b=["de","en"],$="en";function A({initialLang:e}){let s=b.includes(e)?e:$,t=[];function n(o){return w[s]?.[o]??w[$]?.[o]??o}function i(){return s}function c(o){!b.includes(o)||o===s||(s=o,t.forEach(d=>d(s)))}function p(o){t.push(o)}function g(){return b}return{t:n,getLang:i,setLang:c,onLangChange:p,getSupportedLanguages:g}}function x(e){return e.includes("Firefox/")?"Firefox "+e.split("Firefox/")[1].split(" ")[0]:e.includes("Edg/")?"Edge "+e.split("Edg/")[1].split(" ")[0]:e.includes("Chrome/")?"Chrome "+e.split("Chrome/")[1].split(" ")[0]:e.includes("Safari/")&&e.includes("Version/")?"Safari "+e.split("Version/")[1].split(" ")[0]:"Unknown"}function C(e){return e.includes("Windows NT")?"Windows":e.includes("Mac OS X")?"macOS":e.includes("Android")?"Android":e.includes("iPhone")||e.includes("iPad")?"iOS":e.includes("Linux")?"Linux":"Unknown"}function E({source:e,language:s}){const t=navigator.userAgent;return{source:e,browser:x(t),os:C(t),screen:`${screen.width}×${screen.height}`,viewport:`${window.innerWidth}×${window.innerHeight}`,language:s,referrer:document.referrer||"(none)",timestamp:new Date().toISOString(),userAgent:t}}function T({apiBase:e}){async function s(t){const n=await fetch(`${e}/issues.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}),i=await n.json();if(!n.ok)throw new Error(i.error||`HTTP ${n.status}`);return i}return{submitIssue:s}}function r(e){const s=document.createElement("div");return s.textContent=e,s.innerHTML}const W="https://github.com/lbatschelet/feelvonroll/issues",F="https://github.com/lbatschelet/feelvonroll/blob/main/LICENSE";function P(e,{debugInfo:s,t,languages:n,currentLang:i,onLangChange:c,onSubmit:p}){const g=new Date().getFullYear();e.innerHTML=`
  ...
  `,D({t,languages:n,currentLang:i,onLangChange:c,onSubmit:p})}function D({t:e,languages:s,currentLang:t,onLangChange:n,onSubmit:i}){const c=document.getElementById("category"),p=document.getElementById("stepsGroup"),g=document.getElementById("title"),o=document.getElementById("description");function d(){const a=c.value;g.placeholder=e(`title.placeholder.${a}`),o.placeholder=e(`description.placeholder.${a}`),p.style.display=a==="bug"?"":"none"}c.addEventListener("change",d),d(),document.getElementById("langToggle").addEventListener("click",()=>{const a=s.find(f=>f!==t)||s[0];n(a)});const u=document.getElementById("issueForm"),h=document.getElementById("submitBtn"),m=document.getElementById("formError");u.addEventListener("submit",async a=>{a.preventDefault(),m.style.display="none";const f=u.title.value.trim(),v=u.description.value.trim();if(!f||!v){L(m,e("error.required"));return}const I={category:u.category.value,title:f,description:v,steps:u.steps?.value.trim()||"",website:u.website.value};h.disabled=!0,h.classList.add("loading");try{await i(I)}catch(k){L(m,k.message||e("error.generic")),h.disabled=!1,h.classList.remove("loading")}})}function L(e,s){e.textContent=s,e.style.display=""}function R(e,{t:s,onSubmitAnother:t}){e.innerHTML=`
